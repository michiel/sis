...
fn extract_severity_features(findings: &[Finding]) -> SeverityFeatures {
    ...
    features.max_severity_actions = per_surface_severity.get(&AttackSurface::Actions).copied().unwrap_or(0.0);
    features.max_severity_javascript = per_surface_severity.get(&AttackSurface::JavaScript).copied().unwrap_or(0.0);
    features.max_severity_streams = per_surface_severity.get(&AttackSurface::StreamsAndFilters).copied().unwrap_or(0.0);
    features.max_severity_embedded = per_surface_severity.get(&AttackSurface::EmbeddedFiles).copied().unwrap_or(0.0);
    features.max_severity_structural = per_surface_severity.get(&AttackSurface::FileStructure).copied().unwrap_or(0.0);
    features.max_severity_forms = per_surface_severity.get(&AttackSurface::Forms).copied().unwrap_or(0.0);
    features.max_severity_crypto = per_surface_severity.get(&AttackSurface::CryptoSignatures).copied().unwrap_or(0.0);
...
fn extract_finding_features(findings: &[Finding]) -> (FindingPresenceFeatures, FindingCountFeatures) {
    ...
    set_finding!("incremental_update_chain", incremental_update_chain, incremental_update_chain_count);
    set_finding!("object_id_shadowing", object_id_shadowing, object_id_shadowing_count);
    set_finding!("polyglot_signature_conflict", polyglot_signature_conflict, polyglot_signature_conflict_count);
    ...
    set_finding!("decompression_ratio_suspicious", decompression_ratio_suspicious, decompression_ratio_suspicious_count);
    ...
    set_finding!("external_action_risk_context", external_action_risk_context, external_action_risk_context_count);
    ...
    set_finding!("js_runtime_network_intent", js_runtime_network_intent, js_runtime_network_intent_count);
    ...
    if finding_counts_map.contains_key("js_sandbox_exec") ||
       ...
       finding_counts_map.contains_key("js_sandbox_skipped") {
        ...
        counts.js_sandbox_result_count = finding_counts_map.get("js_sandbox_exec").copied().unwrap_or(0) as f32 +
    ...
    set_finding!("fontmatrix_payload_present", fontmatrix_payload_present, fontmatrix_payload_present_count);
    ...
    set_finding!("quantum_vulnerable_crypto", quantum_vulnerable_crypto, quantum_vulnerable_crypto_count);
    ...
    // Supply chain (3)
    set_finding!("supply_chain_persistence", supply_chain_persistence, supply_chain_persistence_count);
    set_finding!("supply_chain_staged_payload", supply_chain_staged_payload, supply_chain_staged_payload_count);
    set_finding!("supply_chain_update_vector", supply_chain_update_vector, supply_chain_update_vector_count);
...
fn extract_uri_features(findings: &[Finding]) -> UriSignalFeatures {
    ...
    features.external_dependency_count = features.http_count + features.https_count + features.file_uri_count;
...
fn extract_content_features(findings: &[Finding]) -> ContentSignalFeatures {
    ...
    for finding in findings {
        ...
        if finding.kind == "invisible_text" || meta.get("content.invisible_text").map(|s| s == "true").unwrap_or(false) {
        ...
        // Image-only pages
        if finding.kind == "image_only_page" || meta.get("content.image_only").map(|s| s == "true").unwrap_or(false) {
        ...
        // Hidden annotations
        if finding.kind == "hidden_annotation" || meta.get("annotation.hidden").map(|s| s == "true").unwrap_or(false) {
            features.hidden_annotation_count += 1.0;
        }
        ...
        // Color manipulation
        if finding.kind.contains("color_manipulation") || meta.contains_key("content.color_manipulation") {
            features.color_manipulation_count += 1.0;
        }

        // Whitespace abuse
        if finding.kind.contains("whitespace_abuse") || meta.contains_key("content.whitespace_abuse") {
        ...
        if finding.kind.contains("layer") || finding.kind.contains("ocg") || meta.contains_key("content.layer_manipulation") {
...
fn extract_supply_chain_features(findings: &[Finding]) -> SupplyChainFeatures {
    ...
    for finding in findings {
        ...
        if finding.kind.contains("c2") || finding.kind.contains("command_control") || meta.contains_key("supply_chain.c2") {
        ...
        if finding.kind.contains("anti_forensics") || meta.contains_key("supply_chain.anti_forensics") {
...
fn extract_structural_features(findings: &[Finding]) -> StructuralAnomalyFeatures {
    ...
    for finding in findings {
        ...
        if finding.kind.contains("stream_length_mismatch") || finding.kind.contains("dictionary_mismatch") {
...
fn extract_crypto_features(findings: &[Finding]) -> CryptoFeatures {
    ...
    for finding in findings {
        ...
        // Weak algorithms
        if finding.kind.contains("weak_algo") || meta.get("crypto.weak_algorithm").map(|s| s == "true").unwrap_or(false) {
            features.weak_algo_count += 1.0;
        }

        // Quantum vulnerability
        if finding.kind.contains("quantum") || meta.get("crypto.quantum_vulnerable").map(|s| s == "true").unwrap_or(false) {
        ...
        if finding.kind.contains("signature_validation") || finding.kind.contains("invalid_signature") {
...
fn extract_embedded_features(findings: &[Finding]) -> EmbeddedContentFeatures {
    ...
    for finding in findings {
        ...
        // PE executables
        if finding.kind.contains("pe_executable") || meta.get("embedded.type").map(|s| s == "pe").unwrap_or(false) {
            features.pe_executable_count += 1.0;
        }

        // Encrypted containers
        if finding.kind.contains("encrypted") || meta.get("embedded.encrypted").map(|s| s == "true").unwrap_or(false) {
        ...
        // 3D content
        if finding.kind.contains("3d") || finding.kind.contains("u3d") || finding.kind.contains("prc") {
        ...
        // Archives
        if finding.kind.contains("archive") || meta.get("embedded.is_archive").map(|s| s == "true").unwrap_or(false) {
        ...
        // Macro-enabled files
        if finding.kind.contains("macro") || meta.get("embedded.has_macros").map(|s| s == "true").unwrap_or(false) {
