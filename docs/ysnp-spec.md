...
## 2) Attack vectors (what ysnp should detect)
...
## 3) Technical specification: ysnp architecture
...
ysnp/
  ...
  crates/
    ysnp-core/        // parser abstractions, model, detectors
    ysnp-pdf/         // PDF parsing + object graph + decoding
    ysnp-detectors/   // rules
    ysnp-cli/         // clap CLI + json output
    ysnp-tui/         // optional ratatui UI
```
...
```rust
// ysnp-cli/src/main.rs
use clap::Parser;
use ysnp_core::{scan::ScanOptions, runner::run_scan};
...
Alright — here’s the **ysnp-pdf parser spec + repo skeleton + real detectors**, written so you can actually start coding immediately.
...
# A) ysnp-pdf parsing spec (detailed)
...
## C1) ysnp-pdf: byte cursor + lexer (core)

```rust
// crates/ysnp-pdf/src/lexer.rs
use crate::span::Span;
...
// --- Types referenced above (put in model module in ysnp-pdf) ---
...
```rust
// crates/ysnp-detectors/src/actions.rs
use ysnp_core::model::*;
use ysnp_core::detect::*;
use anyhow::Result;
...
This assumes you’ve implemented in `ysnp-pdf`:
...
```rust
// crates/ysnp-detectors/src/embedded.rs
use ysnp_core::model::*;
use ysnp_core::detect::*;
use anyhow::Result;
...
# F) Wiring it up (ysnp-core registry)

```rust
// crates/ysnp-core/src/registry.rs
use crate::detect::Detector;

pub fn default_detectors() -> Vec<Box<dyn Detector>> {
    vec![
        Box::new(ysnp_detectors::actions::OpenActionDetector),
        Box::new(ysnp_detectors::embedded::EmbeddedFileDetector),
...
## 1) ysnp-pdf: graph helpers + minimal parse entrypoint

### `crates/ysnp-pdf/src/lib.rs`
...
## 2) ysnp-pdf: token buffer (peek) + object model

### `crates/ysnp-pdf/src/tokbuf.rs`
...
## 4) ysnp-pdf: classic `startxref` + trailer parser

### `crates/ysnp-pdf/src/xref.rs`
...
## 5) ysnp-pdf: recovery scanner that builds the object graph

### `crates/ysnp-pdf/src/recover.rs`
...
## 6) ysnp-pdf: ObjectGraph + helper queries for detectors
...
## 7) ysnp-detectors: working JavaScriptDetector
...
### `crates/ysnp-detectors/src/javascript.rs`
...
impl Detector for JavaScriptDetector {
    ...
    fn run(&self, ctx: &ysnp_core::scan::ScanContext) -> Result<Vec<Finding>> {
        ...
        for e in g.iter_objects() {
            ...
            if let Some((k, v)) = ysnp_pdf::graph::ObjectGraph::dict_get_first(d, b"/S") {
            ...
            if let Some((k, v)) = ysnp_pdf::graph::ObjectGraph::dict_get_first(d, b"/JS") {
        ...
        if let Some(cat_r) = g.catalog_ref() {
            if let Some(cat_e) = g.get(cat_r) {
                if let Some(cat_d) = g.obj_as_dict(cat_e) {
                    if let Some((_, names_obj)) = ysnp_pdf::graph::ObjectGraph::dict_get_first(cat_d, b"/Names") {
                        if let Some(names_e) = g.resolve_ref(names_obj) {
                            if let Some(names_d) = g.obj_as_dict(names_e) {
                                if let Some((_, js_names_obj)) = ysnp_pdf::graph::ObjectGraph::dict_get_first(names_d, b"/JavaScript") {
                                    // could be ref or dict
                                    let js_d_opt = match &js_names_obj.atom {
                                        ysnp_pdf::object::PdfAtom::Ref{..} => g.resolve_ref(js_names_obj).and_then(|e| g.obj_as_dict(e)),
                                        ysnp_pdf::object::PdfAtom::Dict(d) => Some(d),
                                        _ => None,
                                    };

                                    if let Some(js_d) = js_d_opt {
                                        if ysnp_pdf::graph::ObjectGraph::dict_get_first(js_d, b"/Names").is_some() {
...
### Update `crates/ysnp-core/src/registry.rs`
...
pub fn default_detectors() -> Vec<Box<dyn Detector>> {
    vec![
        Box::new(ysnp_detectors::actions::OpenActionDetector),
        Box::new(ysnp_detectors::javascript::JavaScriptDetector),
        Box::new(ysnp_detectors::embedded::EmbeddedFileDetector),
    ]
}
```
...
## 8) ysnp-core: make ScanContext use ysnp-pdf graph

Make sure `ysnp-core` uses the concrete graph type.
...
pub struct ScanContext<'a> {
    pub bytes: &'a [u8],
    pub graph: ysnp_pdf::graph::ObjectGraph<'a>,
    pub options: ScanOptions,
}
```

### `crates/ysnp-core/src/runner.rs` (minimal)
...
pub fn run_scan(bytes: &[u8], options: ScanOptions) -> anyhow::Result<Report> {
    let graph = ysnp_pdf::parse(bytes, options.recover_xref)?;
...
### `crates/ysnp-detectors/tests/js_detector.rs`

```rust
use ysnp_core::scan::ScanOptions;
use ysnp_core::runner::run_scan;
...
members = [
  "crates/ysnp-core",
  "crates/ysnp-pdf",
  "crates/ysnp-detectors",
  "crates/ysnp-cli",
]
resolver = "2"
```

### `crates/ysnp-core/Cargo.toml`
...
rayon = "1"
ysnp-pdf = { path = "../ysnp-pdf" }
ysnp-detectors = { path = "../ysnp-detectors" }
```

### `crates/ysnp-pdf/Cargo.toml`
...
### `crates/ysnp-detectors/Cargo.toml`
...
(You may need to split `ysnp-core` traits/model into a separate crate later to avoid circular deps; for now this works if you keep the layering consistent. If Cargo complains about cycles, the fix is: move `model.rs` + `detect.rs` into `ysnp-api` crate and depend on that from both `ysnp-core` and `ysnp-detectors`.)

---

# What you’ll get immediately

* `cargo test -p ysnp-detectors` should pass and prove:

  * JS action detection works
  * JS in Names tree detection works
* `ysnp-pdf` can parse and build an object index via recovery scan
...
3. **Stream span extraction + Flate decode quota** (ysnp-pdf side)

I’ll paste this as “files to add/replace”.

---

# 1) ysnp-pdf: add stream extraction in recovery + safe Flate decode
...
## 1.2 New file: `crates/ysnp-pdf/src/stream.rs`
...
## 1.3 Update `crates/ysnp-pdf/src/lib.rs`
...
## 1.4 Replace `crates/ysnp-pdf/src/recover.rs` with stream-span support
...
# 2) ysnp-pdf: add helpers for streams/filters + JS extraction

## 2.1 Update `crates/ysnp-pdf/src/graph.rs` (append these helpers)
...
# 3) ysnp-detectors: OpenAction resolver/classifier + AA detector

## 3.1 New file: `crates/ysnp-detectors/src/openaction.rs`
...
impl Detector for OpenActionClassifierDetector {
    ...
    fn run(&self, ctx: &ysnp_core::scan::ScanContext) -> Result<Vec<Finding>> {
        ...
        let Some((k_name, open_obj)) = ysnp_pdf::graph::ObjectGraph::dict_get_first(cat_d, b"/OpenAction") else {
            return Ok(out);
        };

        // resolve action dict (could be ref or inline dict)
        let action_dict = match &open_obj.atom {
            ysnp_pdf::object::PdfAtom::Ref{..} => g.resolve_ref(open_obj).and_then(|e| g.obj_as_dict(e)),
            ysnp_pdf::object::PdfAtom::Dict(d) => Some(d),
            _ => None,
        };
        ...
        if let Some(ad) = action_dict {
            // classify by /S
            if let Some((_, s_obj)) = ysnp_pdf::graph::ObjectGraph::dict_get_first(ad, b"/S") {
                if let ysnp_pdf::object::PdfAtom::Name(n) = &s_obj.atom {
                    match n.decoded.as_slice() {
                        b"/JavaScript" => {
                            ...
                            if let Some((_, js_obj)) = ysnp_pdf::graph::ObjectGraph::dict_get_first(ad, b"/JS") {
            ...
            if let ysnp_pdf::object::PdfAtom::Ref{obj, gen} = open_obj.atom {
...
## 3.2 New file: `crates/ysnp-detectors/src/aa.rs`
...
impl Detector for AdditionalActionsDetector {
    ...
    fn run(&self, ctx: &ysnp_core::scan::ScanContext) -> Result<Vec<Finding>> {
        ...
        for e in g.iter_objects() {
            let Some(d) = g.obj_as_dict(e) else { continue; };
            let hits = ysnp_pdf::graph::ObjectGraph::dict_get_all(d, b"/AA");
            ...
            for (k, v) in hits {
                ...
                let aa_dict = match &v.atom {
                    ysnp_pdf::object::PdfAtom::Dict(dd) => Some(dd),
                    ysnp_pdf::object::PdfAtom::Ref{..} => g.resolve_ref(v).and_then(|er| g.obj_as_dict(er)),
                    _ => None,
                };
...
fn aa_contains_javascript<'a>(
    g: &ysnp_pdf::graph::ObjectGraph<'a>,
    aa_dict: &ysnp_pdf::object::PdfDict<'a>,
    file: &[u8],
    max_decode: usize
) -> bool {
    for (_k, v) in &aa_dict.entries {
        ...
        let ad = match &v.atom {
            ysnp_pdf::object::PdfAtom::Dict(d) => Some(d),
            ysnp_pdf::object::PdfAtom::Ref{..} => g.resolve_ref(v).and_then(|e| g.obj_as_dict(e)),
            _ => None,
        };
...
## 3.3 Update `crates/ysnp-detectors/src/lib.rs`
...
## 3.4 Update `crates/ysnp-core/src/registry.rs`
...
pub fn default_detectors() -> Vec<Box<dyn Detector>> {
    vec![
        Box::new(ysnp_detectors::actions::OpenActionDetector),
        Box::new(ysnp_detectors::openaction::OpenActionClassifierDetector), // NEW
        Box::new(ysnp_detectors::aa::AdditionalActionsDetector),            // NEW
        Box::new(ysnp_detectors::javascript::JavaScriptDetector),
        Box::new(ysnp_detectors::embedded::EmbeddedFileDetector),
    ]
}
...
### `crates/ysnp-detectors/tests/openaction_and_aa.rs`
...
# 1) ysnp-pdf: filter-chain decoding (safe + quota-limited)
...
## 1.2 New file: `crates/ysnp-pdf/src/decode/mod.rs`
...
## 1.3 New file: `crates/ysnp-pdf/src/decode/asciihex.rs`
...
## 1.4 New file: `crates/ysnp-pdf/src/decode/runlength.rs`
...
## 1.5 New file: `crates/ysnp-pdf/src/decode/ascii85.rs`
...
## 1.6 Update `crates/ysnp-pdf/src/lib.rs` to export decode module
...
# 2) ysnp-pdf: upgrade JS extraction to support filter chains

Update the `extract_js_payload` helper you added earlier in `crates/ysnp-pdf/src/graph.rs`.
...
impl Detector for NetworkActionDetector {
    ...
    fn run(&self, ctx: &ysnp_core::scan::ScanContext) -> Result<Vec<Finding>> {
        ...
        for e in g.iter_objects() {
            ...
            // find /S
            let Some((_, s_obj)) = ysnp_pdf::graph::ObjectGraph::dict_get_first(d, b"/S") else { continue; };
            let s_name = match &s_obj.atom {
                ysnp_pdf::object::PdfAtom::Name(n) => n.decoded.clone(),
                _ => continue,
            };

            if s_name.as_slice() == b"/URI" {
                if let Some((k, uri_obj)) = ysnp_pdf::graph::ObjectGraph::dict_get_first(d, b"/URI") {
                ...
                if let Some((k, f_obj)) = ysnp_pdf::graph::ObjectGraph::dict_get_first(d, b"/F") {
...
fn extract_textish(o: &ysnp_pdf::object::PdfObj<'_>) -> Option<String> {
    use ysnp_pdf::object::{PdfAtom, PdfStr};
    match &o.atom {
...
```rust
        Box::new(ysnp_detectors::network::NetworkActionDetector), // NEW
```

---

# 4) ysnp-detectors: ObjStm index-only detector

This flags `/Type /ObjStm`, reports `/N` and `/First`, stream size (raw), and presence of risky filters.

## 4.1 New file: `crates/ysnp-detectors/src/objstm.rs`
...
fn int_first(d: &ysnp_pdf::object::PdfDict<'_>, key: &[u8]) -> Option<i64> {
    let (_, v) = ysnp_pdf::graph::ObjectGraph::dict_get_first(d, key)?;
    if let ysnp_pdf::object::PdfAtom::Int(n) = v.atom { Some(n) } else { None }
}
```
...
```rust
        Box::new(ysnp_detectors::objstm::ObjStmDetector), // NEW
```
...
### `crates/ysnp-detectors/tests/network_and_objstm.rs`
...
# 6) One small wiring fix: ensure `ysnp-detectors` sees new modules

### `crates/ysnp-detectors/src/lib.rs` (complete example)
...
### Replace `crates/ysnp-core/src/runner.rs` with this version
...
# 1) ysnp-pdf: Name tree enumeration (real `/Names` + `/Kids`)

## 1.1 New file: `crates/ysnp-pdf/src/nametree.rs`
...
### Update `crates/ysnp-pdf/src/lib.rs`
...
# 2) ysnp-detectors: Deep ObjStm enumeration (decode + parse table + parse embedded objs)

This detector runs only with `--deep` (Cost::Expensive, now enforced by runner).

## 2.1 New file: `crates/ysnp-detectors/src/objstm_deep.rs`
...
impl Detector for ObjStmEnumerateDetector {
    ...
    fn run(&self, ctx: &ysnp_core::scan::ScanContext) -> Result<Vec<Finding>> {
        ...
        for e in g.iter_objects() {
            ...
            let decoded = ysnp_pdf::decode::decode_with_filters_limited(
                raw_stream,
                &filters,
                ysnp_pdf::decode::DecodePlan{
            ...
            for (objnum, off) in table {
                ...
                let parsed: PdfObj = match ysnp_pdf::object::parse_object(&mut tb) {
                    Ok(o) => o,
                    Err(_) => continue,
                };
...
fn contains_javascript_action(o: &PdfObj<'_>) -> bool {
    ...
    match &o.atom {
        PdfAtom::Dict(d) => {
            let has_js_key = ysnp_pdf::graph::ObjectGraph::dict_get_first(d, b"/JS").is_some();
            let has_s_js = ysnp_pdf::graph::ObjectGraph::dict_get_all(d, b"/S").iter().any(|(_,v)| {
...
```rust
        Box::new(ysnp_detectors::objstm_deep::ObjStmEnumerateDetector), // NEW (Expensive)
```

---

# 3) ysnp-detectors: Action graph detector (trigger → action → payload)
...
## 3.1 New file: `crates/ysnp-detectors/src/action_graph.rs`

```rust
use anyhow::Result;
use ysnp_core::detect::*;
use ysnp_core::model::*;
use ysnp_pdf::object::{PdfAtom, PdfObj, PdfDict};
...
impl Detector for ActionGraphDetector {
    ...
    fn run(&self, ctx: &ysnp_core::scan::ScanContext) -> Result<Vec<Finding>> {
        ...
        for e in g.iter_objects() {
            ...
            if let Some((k, v)) = ysnp_pdf::graph::ObjectGraph::dict_get_first(d, b"/OpenAction") {
            ...
            if let Some((k, v)) = ysnp_pdf::graph::ObjectGraph::dict_get_first(d, b"/A") {
            ...
            if let Some((k, v)) = ysnp_pdf::graph::ObjectGraph::dict_get_first(d, b"/AA") {
...
fn action_type(d: &PdfDict<'_>) -> String {
    if let Some((_, s_obj)) = ysnp_pdf::graph::ObjectGraph::dict_get_first(d, b"/S") {
...
fn action_payload_hint<'a>(
    ...
    // /JS snippet
    if let Some((_, js_obj)) = ysnp_pdf::graph::ObjectGraph::dict_get_first(d, b"/JS") {
    ...
    // /URI value
    if let Some((_, uri_obj)) = ysnp_pdf::graph::ObjectGraph::dict_get_first(d, b"/URI") {
        return Some(format!("[/URI {}]", textish(uri_obj)));
    }
    // /F submit target
    if let Some((_, f_obj)) = ysnp_pdf::graph::ObjectGraph::dict_get_first(d, b"/F") {
        return Some(format!("[/F {}]", textish(f_obj)));
    }
    // /Launch or other keys
    if ysnp_pdf::graph::ObjectGraph::dict_get_first(d, b"/Launch").is_some() {
...
```rust
        Box::new(ysnp_detectors::action_graph::ActionGraphDetector), // NEW
```
...
### Replace `crates/ysnp-detectors/src/javascript.rs` with this upgraded version

```rust
use anyhow::Result;
use ysnp_core::detect::*;
use ysnp_core::model::*;
use ysnp_pdf::object::{PdfAtom};
                ...
                if let Some(cat_d) = g.obj_as_dict(cat_e) {
                    if let Some((_, names_obj)) = ysnp_pdf::graph::ObjectGraph::dict_get_first(cat_d, b"/Names") {
                        ...
                        if let Some(names_d) = names_d {
                            if let Some((_, js_tree_obj)) = ysnp_pdf::graph::ObjectGraph::dict_get_first(names_d, b"/JavaScript") {
                                // enumerate actual entries
                                let entries = ysnp_pdf::nametree::enumerate_name_tree(g, js_tree_obj, 12, 200);
...
### `crates/ysnp-detectors/src/lib.rs` should now include:
...
### `crates/ysnp-core/src/registry.rs` should include these additional detectors:

```rust
        Box::new(ysnp_detectors::action_graph::ActionGraphDetector),
        Box::new(ysnp_detectors::objstm_deep::ObjStmEnumerateDetector), // Expensive (deep only)
```
...
### `crates/ysnp-detectors/tests/action_graph_and_objstm_deep.rs`
...
# 1) ysnp-pdf: Page tree traversal

## 1.1 New file: `crates/ysnp-pdf/src/pagetree.rs`
...
# 2) ysnp-detectors: Annotation action detector (uses page tree)

This finds action triggers hiding in `/Annots` via `/A` and `/AA`.

## 2.1 New file: `crates/ysnp-detectors/src/annots.rs`
...
impl Detector for AnnotationActionDetector {
    ...
    fn run(&self, ctx: &ysnp_core::scan::ScanContext) -> Result<Vec<Finding>> {
        ...
        let pages = ysnp_pdf::pagetree::collect_pages(g, 2000, 64);
        ...
        for p in pages {
            for a in p.annots {
                ...
                // /A
                if let Some((k, v)) = ysnp_pdf::graph::ObjectGraph::dict_get_first(ad, b"/A") {
                ...
                // /AA
                if let Some((k, v)) = ysnp_pdf::graph::ObjectGraph::dict_get_first(ad, b"/AA") {
                    out.push(Finding{
...
fn describe_action<'a>(
    ...
    let s = if let Some((_, s_obj)) = ysnp_pdf::graph::ObjectGraph::dict_get_first(ad, b"/S") {
        if let PdfAtom::Name(n) = &s_obj.atom {
    ...
    // payload hint
    let payload = if let Some((_, js_obj)) = ysnp_pdf::graph::ObjectGraph::dict_get_first(ad, b"/JS") {
    ...
    } else if let Some((_, uri_obj)) = ysnp_pdf::graph::ObjectGraph::dict_get_first(ad, b"/URI") {
        format!("[/URI {}]", textish(uri_obj))
    } else if let Some((_, f_obj)) = ysnp_pdf::graph::ObjectGraph::dict_get_first(ad, b"/F") {
...
```rust
        Box::new(ysnp_detectors::annots::AnnotationActionDetector), // NEW
```

---

# 3) ysnp-pdf: incremental update (/Prev) + startxref chain parsing
...
## 3.1 New file: `crates/ysnp-pdf/src/incremental.rs`
...
# 4) ysnp-detectors: incremental timeline + anomalies detector

## 4.1 New file: `crates/ysnp-detectors/src/incremental.rs`
...
impl Detector for IncrementalUpdateDetector {
    ...
    fn run(&self, ctx: &ysnp_core::scan::ScanContext) -> Result<Vec<Finding>> {
        let bytes = ctx.bytes;
        let revs = ysnp_pdf::incremental::parse_revisions(bytes, 64);
        ...
        if revs.iter().any(|r| matches!(r.kind, ysnp_pdf::incremental::XrefKind::Unknown)) {
            flags.push("unknown_xref_kind");
        }
        if revs.iter().any(|r| matches!(r.kind, ysnp_pdf::incremental::XrefKind::XrefStream)) {
...
```rust
        Box::new(ysnp_detectors::incremental::IncrementalUpdateDetector), // NEW
```
...
## 5.1 New file: `crates/ysnp-detectors/src/shadowing.rs`
...
```rust
        Box::new(ysnp_detectors::shadowing::ObjectShadowingDetector), // NEW
```
...
## 6.1 New file: `crates/ysnp-detectors/src/xref_stream.rs`
...
impl Detector for XrefStreamDetector {
    ...
    fn run(&self, ctx: &ysnp_core::scan::ScanContext) -> Result<Vec<Finding>> {
        ...
        let has_xref_stream = revs.iter().any(|r| matches!(r.kind, ysnp_pdf::incremental::XrefKind::XrefStream));
        if !has_xref_stream {
            return Ok(vec![]);
        }
        ...
        for r in &revs {
            if !matches!(r.kind, ysnp_pdf::incremental::XrefKind::XrefStream) { continue; }
...
```rust
        Box::new(ysnp_detectors::xref_stream::XrefStreamDetector), // NEW
```
...
### `crates/ysnp-detectors/src/lib.rs` should now include (additions in bold):
...
### `crates/ysnp-pdf/src/lib.rs` should now include (additions in bold):
...
### `crates/ysnp-detectors/tests/annots_shadowing_incremental.rs`
...
# 1) ysnp-pdf: xref stream index parser (decode `/Type /XRef` stream)

## 1.1 New file: `crates/ysnp-pdf/src/xrefstream.rs`
...
# 2) ysnp-pdf: helper to find object by start offset

This makes it easy to map `startxref` → recovered object entry.

### Update `crates/ysnp-pdf/src/graph.rs` (add inside `impl ObjectGraph<'a>`)
...
# 3) ysnp-detectors: xref stream index + recovered-vs-xref comparison

This is the core “hidden object” signal generator.

## 3.1 New file: `crates/ysnp-detectors/src/xref_index.rs`
...
impl Detector for XrefStreamIndexCompareDetector {
    ...
    fn run(&self, ctx: &ysnp_core::scan::ScanContext) -> Result<Vec<Finding>> {
        ...
        let revs = ysnp_pdf::incremental::parse_revisions(file, 64);
        let latest_xref_stream = revs.iter().find(|r| matches!(r.kind, ysnp_pdf::incremental::XrefKind::XrefStream));
        ...
        // Build index
        let Some(index) = ysnp_pdf::xrefstream::parse_xref_stream_index(
            g, xref_obj, file,
        ...
        for (objno, ent) in &index.entries {
            match ent {
                ysnp_pdf::xrefstream::XrefEntry::InUse{..} | ysnp_pdf::xrefstream::XrefEntry::Compressed{..} => {
                    if !recovered.contains(objno) {
                        missing.push(*objno);
                        if matches!(ent, ysnp_pdf::xrefstream::XrefEntry::Compressed{..}) { compressed_missing += 1; }
                    }
                }
                _ => {}
            }
...
```rust
        Box::new(ysnp_detectors::xref_index::XrefStreamIndexCompareDetector), // NEW (Expensive)
```
...
## 4.1 New file: `crates/ysnp-detectors/src/incremental_diff.rs`
...
impl Detector for IncrementalHeuristicDiffDetector {
    ...
    fn run(&self, ctx: &ysnp_core::scan::ScanContext) -> Result<Vec<Finding>> {
        let b = ctx.bytes;
        let revs = ysnp_pdf::incremental::parse_revisions(b, 64);
...
```rust
        Box::new(ysnp_detectors::incremental_diff::IncrementalHeuristicDiffDetector), // NEW
```
...
### `crates/ysnp-pdf/src/lib.rs` should now include:
...
### `crates/ysnp-core/src/registry.rs` should now include:

```rust
Box::new(ysnp_detectors::xref_index::XrefStreamIndexCompareDetector),
Box::new(ysnp_detectors::incremental_diff::IncrementalHeuristicDiffDetector),
```
...
### `crates/ysnp-detectors/tests/xref_index_smoke.rs`
...
# 1) ysnp-pdf: ObjStm extractor (decode + parse table + get object by index)

## 1.1 New file: `crates/ysnp-pdf/src/objstm_extract.rs`
...
# 2) ysnp-detectors: type-2 xref → ObjStm extraction detector
...
## 2.1 New file: `crates/ysnp-detectors/src/xref_objstm_extract.rs`
...
impl Detector for XrefType2ObjStmExtractionDetector {
    ...
    fn run(&self, ctx: &ysnp_core::scan::ScanContext) -> Result<Vec<Finding>> {
        ...
        let revs = ysnp_pdf::incremental::parse_revisions(file, 64);
        let latest = revs.iter().find(|r| matches!(r.kind, ysnp_pdf::incremental::XrefKind::XrefStream));
        ...
        for (objno, ent) in &index.entries {
            if let ysnp_pdf::xrefstream::XrefEntry::Compressed{ objstm_obj, objstm_index } = ent {
        ...
        let mut cache: HashMap<u64, ysnp_pdf::objstm_extract::ObjStmDecoded> = HashMap::new();
        ...
        for (objno, objstm_obj, objstm_index) in t2 {
            ...
            let dec = if let Some(d) = cache.get(&objstm_obj) {
                ...
                let d = match ysnp_pdf::objstm_extract::decode_objstm(
                    g,
                    objstm_entry,
                    file,
                    ctx.options.max_decode_bytes.min(64 * 1024 * 1024)
            ...
            let embedded: Option<PdfObj> = ysnp_pdf::objstm_extract::extract_embedded_object_by_index(&dec, idx);
...
fn classify_embedded<'a>(
    ...
    match &o.atom {
        PdfAtom::Dict(d) => {
            // Action dict?
            if g.dict_has_name_value(d, b"/S", b"/JavaScript") || ysnp_pdf::graph::ObjectGraph::dict_get_first(d, b"/JS").is_some() {
            ...
            if g.dict_has_name_value(d, b"/S", b"/URI") || ysnp_pdf::graph::ObjectGraph::dict_get_first(d, b"/URI").is_some() {
                let uri = if let Some((_, u)) = ysnp_pdf::graph::ObjectGraph::dict_get_first(d, b"/URI") {
                    format!("{}", textish(u))
                } else { "(missing)".into() };
                return (Severity::High, "xref_t2_embedded_uri".into(), format!("Embedded dict looks like URI action. URI={}", uri), evidence);
            }

            if g.dict_has_name_value(d, b"/S", b"/SubmitForm") || ysnp_pdf::graph::ObjectGraph::dict_get_first(d, b"/F").is_some() {
                let f = if let Some((_, u)) = ysnp_pdf::graph::ObjectGraph::dict_get_first(d, b"/F") {
                    format!("{}", textish(u))
                } else { "(missing)".into() };
                return (Severity::High, "xref_t2_embedded_submitform".into(), format!("Embedded dict looks like SubmitForm action. F={}", f), evidence);
            }

            if g.dict_has_name_value(d, b"/S", b"/Launch") || ysnp_pdf::graph::ObjectGraph::dict_get_first(d, b"/Launch").is_some() {
...
```rust
        Box::new(ysnp_detectors::xref_objstm_extract::XrefType2ObjStmExtractionDetector), // NEW (Expensive)
```
...
### `crates/ysnp-pdf/tests/objstm_extract_smoke.rs`
...
"#.to_vec();
    ...
    let dec = ysnp_pdf::objstm_extract::decode_objstm(&g, e, &pdf, 1024*1024).unwrap();
    assert!(dec.table.len() >= 1);

    let obj = ysnp_pdf::objstm_extract::extract_embedded_object_by_index(&dec, 0).unwrap();
    // it should be a dict
    match obj.atom {
        ysnp_pdf::object::PdfAtom::Dict(_) => {}
...
# 1) ysnp-core: visitor framework (trait + walker + adapter to `Finding`)

## 1.1 New file: `crates/ysnp-core/src/visitor.rs`
...
pub struct VisitorContext<'a> {
    pub graph: &'a ysnp_pdf::graph::ObjectGraph<'a>,
    pub file: &'a [u8],
    ...
    *nodes += 1;
    ...
    if ctx.resolve_refs {
        if let PdfAtom::Ref{obj: ro, gen: rg} = obj.atom {
            ...
            if let Some(e) = ctx.graph.get(ysnp_pdf::object::ObjRef{obj: ro, gen: rg}) {
...
# 2) ysnp-detectors: reusable visitors (JS / network actions / launch-ish)
...
## 2.1 New file: `crates/ysnp-detectors/src/embedded_visitors.rs`

```rust
use ysnp_core::model::*;
use ysnp_core::visitor::{VisitorDetector, VisitorContext, EmbeddedFinding};
use ysnp_pdf::object::{PdfAtom, PdfObj};
...
impl VisitorDetector for EmbeddedNetworkActionVisitor {
    ...
    fn on_node(&mut self, _ctx: &VisitorContext, path: &str, obj: &PdfObj<'_>, out: &mut Vec<EmbeddedFinding>) {
        ...
        let is_uri = ysnp_pdf::graph::ObjectGraph::dict_get_first(d, b"/URI").is_some()
            || _ctx.graph.dict_has_name_value(d, b"/S", b"/URI");
        let is_submit = ysnp_pdf::graph::ObjectGraph::dict_get_first(d, b"/F").is_some()
...
impl VisitorDetector for EmbeddedLaunchVisitor {
    ...
    fn on_node(&mut self, ctx: &VisitorContext, path: &str, obj: &PdfObj<'_>, out: &mut Vec<EmbeddedFinding>) {
        ...
        let is_launch = ctx.graph.dict_has_name_value(d, b"/S", b"/Launch")
            || ysnp_pdf::graph::ObjectGraph::dict_get_first(d, b"/Launch").is_some();
...
### Replace `crates/ysnp-detectors/src/xref_objstm_extract.rs` with this updated version
...
use ysnp_core::detect::*;
use ysnp_core::model::*;
use ysnp_core::visitor::{VisitorContext, EmbeddedOrigin, run_visitors_on_embedded, VisitorDetector};
...
impl Detector for XrefType2ObjStmExtractionDetector {
    ...
    fn run(&self, ctx: &ysnp_core::scan::ScanContext) -> Result<Vec<Finding>> {
        ...
        for (objno, objstm_obj, objstm_index) in t2 {
            ...
            let idx = objstm_index as usize;
            let embedded = ysnp_pdf::objstm_extract::extract_embedded_object_by_index(&dec, idx);
...
### `crates/ysnp-core/tests/visitor_smoke.rs`
...
"#.to_vec();
    ...
    let mut v_js = ysnp_detectors::embedded_visitors::EmbeddedJavaScriptVisitor::new();
...
# 0) Add a glue helper in `ysnp-detectors` so detectors can run visitors easily

## 0.1 New file: `crates/ysnp-detectors/src/visitor_glue.rs`

```rust
use ysnp_core::visitor::{EmbeddedOrigin, VisitorContext, run_visitors_on_embedded, VisitorDetector};
use ysnp_core::model::Finding;
...
### Replace the NameTree section in `crates/ysnp-detectors/src/javascript.rs`
                ...
                if let Some(cat_d) = g.obj_as_dict(cat_e) {
                    if let Some((_, names_obj)) = ysnp_pdf::graph::ObjectGraph::dict_get_first(cat_d, b"/Names") {
                        ...
                        if let Some(names_d) = names_d {
                            if let Some((_, js_tree_obj)) = ysnp_pdf::graph::ObjectGraph::dict_get_first(names_d, b"/JavaScript") {
                                // enumerate actual entries
                                let entries = ysnp_pdf::nametree::enumerate_name_tree(g, js_tree_obj, 12, 500);

                                if !entries.is_empty() {
                                    // Visitor context
                                    let vctx = ysnp_core::visitor::VisitorContext{
                                        graph: g,
                                        file: bytes,
                                        max_decode: ctx.options.max_decode_bytes.min(512*1024),
                                        max_depth: 32,
                                    ...
                                    for (idx, ent) in entries.iter().enumerate() {
                                        ...
                                        // Create origin info
                                        let origin = ysnp_core::visitor::EmbeddedOrigin{
...
### Replace `crates/ysnp-detectors/src/annots.rs` with this upgraded version
    ...
    fn run(&self, ctx: &ysnp_core::scan::ScanContext) -> Result<Vec<Finding>> {
        ...
        for p in pages {
            for a in p.annots {
                ...
                // /A (direct action)
                if let Some((_, v)) = ysnp_pdf::graph::ObjectGraph::dict_get_first(ad, b"/A") {
                ...
                if let Some((_, aa_obj)) = ysnp_pdf::graph::ObjectGraph::dict_get_first(ad, b"/AA") {
...
### Replace `crates/ysnp-detectors/src/objstm_deep.rs` with this upgraded version
...
### `crates/ysnp-detectors/src/lib.rs` includes:
...
### 8.1 New file: `crates/ysnp-pdf/src/streamloc.rs`
...
### 8.3 New file: `crates/ysnp-core/src/evidence_map.rs`
...
#### Update `crates/ysnp-core/src/visitor.rs`
...
#### Update `crates/ysnp-detectors/src/embedded_visitors.rs`
...
#### Update `crates/ysnp-detectors/src/xref_objstm_extract.rs` (chunk 6/7 version)
...
```rust
            ...
            let evidence_map = {
                ...
                if filters.is_empty() {
                    if let Some(loc) = ysnp_pdf::streamloc::locate_stream_data(file, objstm_entry) {
                        Some(ysnp_core::evidence_map::EvidenceMap::IdentityStream{
                            data_start_file_off: loc.data.start,
                            data_end_file_off: loc.data.end,
                        })
                    } else {
                        Some(ysnp_core::evidence_map::EvidenceMap::Unmappable{
...
#### Update `crates/ysnp-detectors/src/objstm_deep.rs` similarly
...
#### A) Annotations (`crates/ysnp-detectors/src/annots.rs`)

When building origin for `/A`:

```rust
trigger: Some(ysnp_core::trigger::TriggerContext{
    kind: ysnp_core::trigger::TriggerKind::AnnotationA,
...
```rust
trigger: Some(ysnp_core::trigger::TriggerContext{
    kind: ysnp_core::trigger::TriggerKind::AnnotationAAEvent,
...
#### B) NameTree JavaScript (`crates/ysnp-detectors/src/javascript.rs`)

When building origin for name tree entry:

```rust
trigger: Some(ysnp_core::trigger::TriggerContext{
    kind: ysnp_core::trigger::TriggerKind::NameTreeJavaScript,
...
```rust
trigger: Some(ysnp_core::trigger::TriggerContext{
    kind: ysnp_core::trigger::TriggerKind::XrefType2Embedded,
...
```rust
trigger: Some(ysnp_core::trigger::TriggerContext{
    kind: ysnp_core::trigger::TriggerKind::ObjStmDeep,
    path: "$.ObjStm[]".into(),
    event: None,
...
## New file: `crates/ysnp-detectors/src/trigger_graph.rs`
...
impl Detector for TriggerGraphSummaryDetector {
    ...
    fn run(&self, ctx: &ysnp_core::scan::ScanContext) -> Result<Vec<Finding>> {
        ...
        if let Some(cat_r) = g.catalog_ref() {
            if let Some(cat_e) = g.get(cat_r) {
                if let Some(cat_d) = g.obj_as_dict(cat_e) {
                    if let Some((_, oa)) = ysnp_pdf::graph::ObjectGraph::dict_get_first(cat_d, b"/OpenAction") {
                        edges.extend(describe_action_edge(g, "Catalog/OpenAction", oa));
                    }
                    if let Some((_, aa)) = ysnp_pdf::graph::ObjectGraph::dict_get_first(cat_d, b"/AA") {
        ...
        for p in pages {
            for a in p.annots {
                ...
                if let Some((_, ao)) = ysnp_pdf::graph::ObjectGraph::dict_get_first(ad, b"/A") {
                    edges.extend(describe_action_edge(g, "Annot/A", ao));
                }
                if let Some((_, aao)) = ysnp_pdf::graph::ObjectGraph::dict_get_first(ad, b"/AA") {
...
fn resolve_dict<'a>(g: &ysnp_pdf::graph::ObjectGraph<'a>, o: &PdfObj<'a>) -> Option<&'a ysnp_pdf::object::PdfDict<'a>> {
    match &o.atom {
        PdfAtom::Dict(d) => Some(d),
...
fn describe_action_edge<'a>(g: &ysnp_pdf::graph::ObjectGraph<'a>, from: &str, o: &PdfObj<'a>) -> Vec<String> {
    let mut out = Vec::new();
    ...
    let s = if let Some((_, s_obj)) = ysnp_pdf::graph::ObjectGraph::dict_get_first(d, b"/S") {
        if let PdfAtom::Name(n) = &s_obj.atom {
            String::from_utf8_lossy(&n.decoded).to_string()
        } else { "unknown".into() }
    } else { "unknown".into() };
    ...
    for k in [b"/JS".as_slice(), b"/URI".as_slice(), b"/F".as_slice(), b"/Launch".as_slice()] {
        if ysnp_pdf::graph::ObjectGraph::dict_get_first(d, k).is_some() {
...
```rust
Box::new(ysnp_detectors::trigger_graph::TriggerGraphSummaryDetector), // NEW
```
...
### 10.1 Update `crates/ysnp-core/src/scan.rs` (options + budget)
...
### 10.2 New file: `crates/ysnp-core/src/budget.rs`
...
Update `crates/ysnp-core/src/scan.rs` (or wherever `ScanContext` is defined) to include:
...
In `ysnp-core/src/visitor.rs`, add one line inside `walk` near the start:
...
#### New file: `crates/ysnp-core/src/work.rs`
...
```rust
let mut limiter = ysnp_core::work::WorkLimiter::new(ctx.options.budget.max_embedded_objects);
...
...
# 11.1 ysnp-core: canonical graph model + exporters

## 11.1.1 New file: `crates/ysnp-core/src/graph_model.rs`
...
## 11.1.2 New file: `crates/ysnp-core/src/graph_export.rs`
...
fn write_dot<W: Write>(g: &Graph, w: &mut W) -> io::Result<()> {
    let header = if g.directed { "digraph ysnp" } else { "graph ysnp" };
    writeln!(w, "{} {{", header)?;
...
# 11.2 ysnp-core: “path to payload” reconstruction
...
## 11.2.1 New file: `crates/ysnp-core/src/path_recon.rs`
...
# 11.3 ysnp-detectors: build the graph from scan context + visitor hits
...
## 11.3.1 New file: `crates/ysnp-detectors/src/graph_exporter.rs`

```rust
use anyhow::Result;
use ysnp_core::detect::*;
use ysnp_core::model::*;
use ysnp_core::graph_model::*;
use ysnp_pdf::object::{PdfAtom, PdfObj};
...
impl Detector for GraphBuildAndPathReconDetector {
    ...
    fn run(&self, ctx: &ysnp_core::scan::ScanContext) -> Result<Vec<Finding>> {
        ...
        // Annotation triggers
        let pages = ysnp_pdf::pagetree::collect_pages(gpdf, 2000, 64);
        for p in pages {
                    ...
                    if let Some((_, names_obj)) = ysnp_pdf::graph::ObjectGraph::dict_get_first(cat_d, b"/Names") {
                        if let Some(names_d) = resolve_dict(gpdf, names_obj) {
                            if let Some((_, js_tree_obj)) = ysnp_pdf::graph::ObjectGraph::dict_get_first(names_d, b"/JavaScript") {
                                ...
                                let entries = ysnp_pdf::nametree::enumerate_name_tree(gpdf, js_tree_obj, 12, 300);
        ...
        let paths = ysnp_core::path_recon::reconstruct_paths(&g, 200, 10);
        ...
        let mut dot = Vec::new();
        ysnp_core::graph_export::write_graph(&g, ysnp_core::graph_export::GraphFormat::Dot, &mut dot)?;
        let dot_s = String::from_utf8_lossy(&dot);
        let dot_preview = dot_s.lines().take(80).collect::<Vec<_>>().join("\n");

        let mut json = Vec::new();
        ysnp_core::graph_export::write_graph(&g, ysnp_core::graph_export::GraphFormat::Json, &mut json)?;
...
fn add_trigger_action_payload_chain<'a>(
    gpdf: &ysnp_pdf::graph::ObjectGraph<'a>,
    g: &mut Graph,
    container_id: &str,
    trigger_name: &str,
    ...
    for key in [b"/JS".as_slice(), b"/URI".as_slice(), b"/F".as_slice(), b"/Launch".as_slice()] {
        if let Some((_, v)) = ysnp_pdf::graph::ObjectGraph::dict_get_first(ad, key) {
...
```rust
Box::new(ysnp_detectors::graph_exporter::GraphBuildAndPathReconDetector), // NEW
```
...
if let Some(prefix) = &args.export_graph {
    ...
    let graph = ysnp_runner::graph_build::build_trigger_graph(&ctx); // shown below

    let formats = args.graph_formats.split(',').map(|s| s.trim().to_lowercase()).collect::<Vec<_>>();
    for f in formats {
        match f.as_str() {
            "dot" => write_fmt(prefix, "dot", ysnp_core::graph_export::GraphFormat::Dot, &graph)?,
            "gml" => write_fmt(prefix, "gml", ysnp_core::graph_export::GraphFormat::Gml, &graph)?,
            "json" => write_fmt(prefix, "json", ysnp_core::graph_export::GraphFormat::Json, &graph)?,
            _ => {}
        }
    }
}
...
```rust
fn write_fmt(prefix: &str, ext: &str, fmt: ysnp_core::graph_export::GraphFormat, g: &ysnp_core::graph_model::Graph) -> anyhow::Result<()> {
    let path = format!("{}.{}", prefix, ext);
    let mut f = std::fs::File::create(&path)?;
    ysnp_core::graph_export::write_graph(g, fmt, &mut f)?;
    Ok(())
}
```
...
## 11.5.1 New file: `crates/ysnp-runner/src/graph_build.rs`

(If you don’t have a `ysnp-runner` crate, put this in your CLI crate or `ysnp-core` as appropriate.)
...
pub fn build_trigger_graph<'a>(
    ctx: &ysnp_core::scan::ScanContext<'a>,
) -> Graph {
    let gpdf = &ctx.graph;
    let mut g = Graph::new(true);
            ...
            if let Some(cat_d) = gpdf.obj_as_dict(cat_e) {
                if let Some((_, names_obj)) = ysnp_pdf::graph::ObjectGraph::dict_get_first(cat_d, b"/Names") {
                    if let Some(names_d) = resolve_dict(gpdf, names_obj) {
                        if let Some((_, js_tree_obj)) = ysnp_pdf::graph::ObjectGraph::dict_get_first(names_d, b"/JavaScript") {
                            let tnode = g.upsert_node(id_trigger("Names/JavaScript"), NodeKind::Trigger, "NamesTree(JavaScript)".into());
                            let entries = ysnp_pdf::nametree::enumerate_name_tree(gpdf, js_tree_obj, 12, 500);
                            for ent in entries {
                                let nm = String::from_utf8_lossy(&ent.name).to_string();
                                let entry_id = g.upsert_node(id_trigger(&format!("Names/JavaScript/{}", nm)), NodeKind::Trigger, format!("NameEntry({})", nm));
                                g.add_edge(tnode.clone(), entry_id.clone(), EdgeKind::Triggers, "entry".into());
    ...
    // Synthesized paths
    let paths = ysnp_core::path_recon::reconstruct_paths(&g, 500, 12);
    for ph in paths {
        if ph.chain.len() < 2 { continue; }
        let label = chain_label(&g, &ph.chain);
...
If you tell me whether your CLI crate uses `clap` and where the runner lives (`ysnp-cli`, `ysnp`, etc.), I’ll paste the exact diff for the arg parsing + export hook in the correct file locations.


# Chapter 13

Perfect — here’s an **implementation-ready** way to wire **Clap** + graph export into the **`ysnp` binary**, without committing you to a separate “runner crate” yet.

The plan:

* Put the reusable builder in **`ysnp-core`** (or `ysnp-detectors`), but I recommend **`ysnp-core`** so the `ysnp` binary can call it directly.
* Add Clap flags to `ysnp`:
...
## 1) Put the builder in ysnp-core (runner-agnostic)

### 1.1 New file: `crates/ysnp-core/src/graph_build.rs`

This is basically the builder you wanted, but placed in core so your `ysnp` binary can call it.
...
`crates/ysnp-core/src/lib.rs` add:
...
## 2) Clap flags in the ysnp binary

Assuming your binary is something like `crates/ysnp/src/main.rs` or `src/main.rs`.
...
#[derive(Parser, Debug)]
#[command(name = "ysnp", version, about = "PDF security analyzer")]
pub struct Args {
...
Wherever your `ysnp` binary currently does:
...
### 3.1 Add helper in `ysnp` binary (main.rs)

```rust
fn parse_graph_formats(s: &str) -> Vec<ysnp_core::graph_export::GraphFormat> {
    s.split(',')
        .map(|x| x.trim().to_lowercase())
        .filter_map(|x| match x.as_str() {
            "dot" => Some(ysnp_core::graph_export::GraphFormat::Dot),
            "gml" => Some(ysnp_core::graph_export::GraphFormat::Gml),
            "json" => Some(ysnp_core::graph_export::GraphFormat::Json),
            _ => None,
        })
        .collect()
}

fn graph_ext(fmt: &ysnp_core::graph_export::GraphFormat) -> &'static str {
    match fmt {
        ysnp_core::graph_export::GraphFormat::Dot => "dot",
        ysnp_core::graph_export::GraphFormat::Gml => "gml",
        ysnp_core::graph_export::GraphFormat::Json => "json",
    }
}

fn export_graph_files(
    prefix: &str,
    formats: &[ysnp_core::graph_export::GraphFormat],
    g: &ysnp_core::graph_model::Graph,
) -> anyhow::Result<()> {
...
if let Some(prefix) = &args.export_graph {
    ...
    let graph = ysnp_core::graph_build::build_trigger_graph(&ctx, args.graph_include_paths);
...
# 1) Add `meta` to `Finding` (ysnp-core model)

## 1.1 Update: `crates/ysnp-core/src/model.rs`
...
## 2.1 Update: `crates/ysnp-core/src/visitor.rs`
...
## 3.1 Update: `crates/ysnp-detectors/src/embedded_visitors.rs`
...
## 4.1 New file: `crates/ysnp-core/src/graph_attach.rs`
...
# 5) Update the graph export hook in `ysnp` binary
...
## 5.1 In your `ysnp` binary export hook (main.rs)
...
```rust
let mut graph = ysnp_core::graph_build::build_trigger_graph(&ctx, args.graph_include_paths);
ysnp_core::graph_attach::attach_findings(&mut graph, &report.findings);
export_graph_files(prefix, &formats, &graph)?;
```
...
## 1.1 Update: `crates/ysnp-core/src/graph_model.rs`
...
## 2.1 New file: `crates/ysnp-core/src/hashutil.rs`
...
## 3.1 Update: `crates/ysnp-core/src/graph_attach.rs`
...
## 1.1 New file: `crates/ysnp-core/src/meta_refs.rs`
...
Still in `run_visitors_on_embedded` in `crates/ysnp-core/src/visitor.rs`,
...
```rust
if let Some(r) = ysnp_core::meta_refs::dict_ref_value(d, b"/JS") {
...
```rust
if let Some(r) = ysnp_core::meta_refs::dict_ref_value(d, b"/URI") {
    meta.insert("payload_ref".into(), r);
}
```
...
```rust
if let Some(r) = ysnp_core::meta_refs::dict_ref_value(d, b"/F") {
    meta.insert("payload_ref".into(), r);
}
```
...
```rust
if let Some(r) = ysnp_core::meta_refs::dict_ref_value(d, b"/Launch") {
    meta.insert("payload_ref".into(), r);
}
```
...
```rust
use ysnp_core::meta_refs;
```

(or fully qualify `ysnp_core::meta_refs::dict_ref_value`).
...
## 4.1 Annotations: `crates/ysnp-detectors/src/annots.rs`
...
container_refs.insert("page".into(), ysnp_core::meta_refs::fmt_ref(p.page_ref.obj, p.page_ref.gen));
container_refs.insert("annot".into(), ysnp_core::meta_refs::fmt_ref(annot_e.r.obj, annot_e.r.gen));
```
...
```rust
let action_ref = ysnp_core::meta_refs::obj_ref_string(&v);
```
...
## 4.2 NameTree JS: `crates/ysnp-detectors/src/javascript.rs`
...
```rust
let mut container_refs = std::collections::HashMap::new();
container_refs.insert("catalog".into(), ysnp_core::meta_refs::fmt_ref(cat_r.obj, cat_r.gen));

let action_ref = ysnp_core::meta_refs::obj_ref_string(&ent.value);
```
...
## 4.3 Xref type-2: `crates/ysnp-detectors/src/xref_objstm_extract.rs`
...
```rust
let mut container_refs = std::collections::HashMap::new();
container_refs.insert("xref_stream".into(), ysnp_core::meta_refs::fmt_ref(xref_obj.r.obj, xref_obj.r.gen));
container_refs.insert("objstm".into(), ysnp_core::meta_refs::fmt_ref(objstm_entry.r.obj, objstm_entry.r.gen));
```
...
In `ysnp-core/src/visitor.rs` add:
...
## 4.4 ObjStm deep: `crates/ysnp-detectors/src/objstm_deep.rs`

You have `e.r` for ObjStm container. Add:

```rust
let mut container_refs = std::collections::HashMap::new();
container_refs.insert("objstm".into(), ysnp_core::meta_refs::fmt_ref(e.r.obj, e.r.gen));
...
## 5.1 Update: `crates/ysnp-core/src/graph_attach.rs`
...
* `ysnp-core/src/meta_refs.rs`
* `ysnp-core/src/hashutil.rs` (already added earlier)
* `ysnp-core/src/graph_attach.rs` updates (payload instance key/attrs)
* `ysnp-core/src/visitor.rs`:

  * `EmbeddedOrigin` adds `container_refs`, `action_ref`, `origin_meta`
  * `run_visitors_on_embedded` copies these into `Finding.meta`
* `ysnp-detectors/src/embedded_visitors.rs`:
...
## 2.1 New helper in `ysnp-core/src/graph_attach.rs`
...
In `crates/ysnp-core/src/graph_attach.rs`, you previously added:
...
### 1.1 New file: `crates/ysnp-core/src/chain.rs`
...
### 2.1 New file: `crates/ysnp-core/src/chain_synth.rs`
...
### 3.1 New file: `crates/ysnp-core/src/chain_score.rs`
...
This detector runs **after** you’ve built the graph and attached findings (in your `ysnp` binary export path, you already do that for export; but detectors run inside the scan). So we provide both options:
...
#### New file: `crates/ysnp-core/src/chain_render.rs`
...
## 5) Runner integration (ysnp binary): synthesize + score + append to report + add graph edges

### 5.1 Add chain generation after you build the combined graph

In your `ysnp` binary, you already do:
...
```rust
// 1) Synthesize chains
let chains = ysnp_core::chain_synth::synthesize_chains(
    &graph,
    ysnp_core::chain_synth::ChainSynthesisOptions::default(),
);

// 2) Score chains
let weights = ysnp_core::chain_score::ChainScoringWeights::default();
let mut scored = Vec::new();
for c in chains {
    scored.push(ysnp_core::chain_score::score_chain(c, weights.clone()));
}
...
for c in &scored {
    report.findings.push(ysnp_core::chain_render::chain_to_finding(c));
}
...
for c in &scored {
    for pi in c.payload_instances.iter().take(50) {
        graph.add_edge(c.trigger_node.clone(), pi.clone(), ysnp_core::graph_model::EdgeKind::Path, "chain_member".into());
    }
}
```
...
# 1) Add a small JS signal extractor (ysnp-detectors)
...
## 1.1 New file: `crates/ysnp-detectors/src/js_signals.rs`
...
### 2.1 Update: `crates/ysnp-detectors/src/embedded_visitors.rs`

At top:

```rust
use ysnp_detectors::js_signals;
```
...
```rust
        ...
        if let Some((_, js_obj)) = ysnp_pdf::graph::ObjectGraph::dict_get_first(d, b"/JS") {
            if let ysnp_pdf::object::PdfAtom::Str(sv) = &js_obj.atom {
...
```rust
        ...
        if let Some((_, js_obj)) = ysnp_pdf::graph::ObjectGraph::dict_get_first(d, b"/JS") {
            // payload_ref if ref
            if let Some(r) = ysnp_core::meta_refs::obj_ref_string(js_obj) {
                meta.insert("payload_ref".into(), r);
            }
...
### 5.1 Update: `crates/ysnp-core/src/chain_score.rs`
...
### 6.1 Update: `crates/ysnp-core/src/chain_render.rs`
...
## 1.1 Update: `crates/ysnp-core/src/visitor.rs`

Add imports (near top):

```rust
use ysnp_pdf::graph::ObjectGraph;
use ysnp_pdf::object::{PdfAtom, PdfObj};
```
...
impl<'a> VisitorContext<'a> {
    ...
    pub fn resolve<'b>(&'b self, o: &'b PdfObj<'a>) -> Option<ysnp_pdf::graph::ObjEntry<'a>> {
        match o.atom {
    ...
    fn extract_bytes_from_entry(&self, e: ysnp_pdf::graph::ObjEntry<'a>) -> Option<Vec<u8>> {
...
In `crates/ysnp-core/src/visitor.rs`, wherever your visitor trait is defined (you’ve been using `on_node(&self, node, path, out)`), change it to include ctx:
...
pub trait EmbeddedVisitor: Send + Sync {
    ...
    fn on_node<'a>(
        &self,
        vctx: &VisitorContext<'a>,
        node: &ysnp_pdf::visit::DictNode<'a>,
        path: &str,
        out: &mut Vec<EmbeddedFinding>,
    );
}
...
### 3.1 Update: `crates/ysnp-core/src/visitor.rs` (or wherever `run_visitors_on_embedded` lives)
...
### 3.2 In your ysnp binary (or wherever you call visitors)
...
## 4.1 Update: `crates/ysnp-detectors/src/embedded_visitors.rs`
...
fn on_node<'a>(
    &self,
    vctx: &ysnp_core::visitor::VisitorContext<'a>,
    node: &ysnp_pdf::visit::DictNode<'a>,
    path: &str,
    out: &mut Vec<ysnp_core::visitor::EmbeddedFinding>,
) {
   // ...
}
```
...
if let Some((_, js_obj)) = ysnp_pdf::graph::ObjectGraph::dict_get_first(d, b"/JS") {
    ...
    if let Some(js_bytes) = vctx.extract_bytes(js_obj) {
        ...
        meta.insert(
            ...
            match &js_obj.atom {
                ysnp_pdf::object::PdfAtom::Ref{..} => "ref".into(),
                ysnp_pdf::object::PdfAtom::Str(_) => "inline_str".into(),
                ysnp_pdf::object::PdfAtom::Stream(_) => "inline_stream_raw".into(),
                _ => "other".into(),
            }
        );
